...
export const chunkText = (text, chunkSize = 500, overlap = 50) => {
    ...
    for (const paragraph of paragraphs) {
        ...
        if (currentWordCount + paragraphWordCount > chunkSize && currentChunk.length > 0) {
            ...
            const overlap = prevWords.slice(-Math.min(overlap, prevWords.length)).join(' ');
    ...
    if (chunks.length === 0 && cleanedText.length > 0) {
        ...
        for (let i = 0; i < allWords.length; i += (chunkSize - overlap)) {
            const chunkWords = allWords.splice(i, i + chunkSize);
            chunks.push({
                content: currentChunk.join(' '),
                chunkIndex: chunkIndex++,
                pageNumber: 0
 ...
 * @param {number} maxchunk - Maximum chunks to return
...
export const findRelevantChunks = (chunks, query, maxchunk = 3) => {
    ...
    const stopWords = new Set([
        'the', 'is', 'at', 'which', 'on', 'a', 'and', 'or', 'but',
